...
**FAMEWS** has primarly been designed to run on HiRID dataset. However, it is possible to give already processed input to some stages in order to run it on other datasets that differ in their format.  
...
After explaining how to set up FAMEWS, we will first describe how to run it on HiRID dataset and then we will give a more detailed documentation that extends its range of applications.
...
The following commands can be used to run a basic configuration of the fairness analysis on HiRID dataset based on our example models. 
...
- `use_multiple_predictions` flag to run the analysis of average of predictions by models trained with different random seeds. The root directory where the predictions are stored and the list of seeds need to be passed in GIN config as attribute of the `HandleMultiplePredictions` stage.
    If `use_multiple_predictions = False` then the pickle file containing the predictions need to be stored in the log directory as `predictions/predictions_split_{split_name}.pkl`. 
...
- `SetUpGroupDef` load the grouping definitons
    In GIN config:
        - `group_config_file` path to the YAML file containing the grouping definition

- `LoadPatientsDf` load or generate the dataframe with patient's demographics
    If it has been pre-computed, you just need to give in the gin config the path at `LoadPatientsDf.patients_df_file`
    Otherwise, in GIN config:
        - `patients_df_file`: path to store the generated dataframe
        - `patients_source`: data source, currently supported `hirid` or `mimic`
        - `general_table_hirid_path`: if data source is `hirid`, path to the extended general table (generated by HiRID preprocessing)
        - `root_mimic`: if data source is `mimic`, path to the directory containing all MIMIC-III dataframes
        - `pid_name` string refering to the patient id, for HiRID it is `patientid` and for MIMIC it is `PatientID`
...
- `LoadEventBounds` load or generate the target event boundaries
    ...
    Otherwise, in GIN config:
        ...
        - `event_column`: string refering to the event column name in the endpoint dataframes
...
- `LoadSplitPids` load or patients splits (train, validation, test) dictionary
    ...
    Otherwise, in GIN config:
        ...
        - `data_table_file`: path of h5 file contaning the split definition, in the same format as the ML-stage preprocessed data of HiRID

- `HandlePredictions` load the predictions that are stored at `{log_dir}/predictions/predictions_split_{split_name}.pkl`
...
- `HandleMultiplePredictions` load multiple predictions and average them. The subsequent analysis will be run on the averaged predictions.
    ...
    - `root_predictions_path`: path of the root directory containing the different predictions
