...
class TestModelPL(PipelineStage):
    ...
    def run(self) -> Any:
        ...
        val_dataloader = DataLoader(
            self.state.val_dataset, batch_size=self.batch_size, shuffle=False, num_workers=1
        )

        test_dataloader = DataLoader(
            test_dataset, batch_size=self.batch_size, shuffle=False, num_workers=1
        )
        ...
        print_and_save_metrics(
            self.state.log_dir, test_metrics[0], prefix="test", print_metrics=True, strip_keys=5
        )
    ...
    def is_done(self) -> bool:
        # TODO: check if metrics have been written to log_dir
        return self.done
