...
from famews.pipeline import GenericPipeline
from famews.train.pipeline import MLTrainPipeline, SetSeeds, SetupGin
...
def parse_arguments(argv=None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "-c", "--columns", default=None, required=False, type=str, nargs="+", help="List columns"
    )
    parser.add_argument(
...
def main(argv=None):
    ...
    mltrainpipeline_constructor = partial(
        MLTrainPipeline,
    ...
    if arguments["columns"] is not None:
        mltrainpipeline_constructor.keywords["columns"] = arguments["columns"]

    train_pipeline = mltrainpipeline_constructor()
    train_pipeline.run()
