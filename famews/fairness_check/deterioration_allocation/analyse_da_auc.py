...
@gin.configurable("AnalyseDAAUC", denylist=["state"])
class AnalyseDAAUC(StatefulPipelineStage):
    """
    Analyse Feature Importance per Group Pipeline stage
    """

    name = "Analyse Deterioration-Allocation Index"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if root_model_path is not None:
            if len(model_seeds):
                self.list_model_paths = [
                    Path(root_model_path) / f"seed_{s}" / model_save_name
        ...
        self.ml_stage_data_path = (
            Path(ml_stage_data_path)
            if ml_stage_data_path is not None
            else ml_stage_data_path
        )
        self.split = split
        ...
        self.feature_names_path = (
            ...
            else feature_names_path
        )

        if len(model_seeds):
            self.list_shap_values_paths = [
                Path(root_shap_values_path)
                / f"seed_{s}"
                / f"{self.split}_shap_values.pkl"
                for s in model_seeds
            ]
        else:
            self.list_shap_values_paths = [
    ...
    def run(self):
        ...
        if self.ml_stage_data_path and self.ml_stage_data_path.exists():
            ...
            feature_names = np.array(
                [f.decode("utf-8") for f in self.data_h5.data.columns[:]]
            )
        ...
        self.state.feat_ranking_all = get_feat_ranking(
            np.mean([avg_shap_value_patients[pid] for pid in shap_pids], axis=0),
            feature_names,
        )
        ...
        for group_name, cats in self.state.groups.items():
            ...
            avg_shap_group = average_shap_values_group(
                map_group_pid, avg_shap_value_patients
            )
