    ...
    def rbo_ext(self, p=0.98):
        ...
        for d in tqdm(range(1, l), disable=~self.verbose):
            if d < s:  # still overlapping in length
                ...
                ext_term = (
                    1.0 * ((X[d] - X_s) / (d + 1) + X[s - 1] / s) * p ** (d + 1)
                )  # last term in Eq. (32)

        return self._bound_range(rbo[-1] + disjoint + ext_term)

    def top_weightness(self, p: Optional[float] = None, d: Optional[int] = None):
        """
        ...
        if d == 0:
            ...
            top_w = (
                1
                - p ** (i)
                + 1.0 * (1 - p) / p * (i + 1) * (np.log(1.0 / (1 - p)) - sum_1)
            )  # here i == d-1

        if self.verbose:
            print(
                "The first {} ranks have {:6.3%} of the weight of "
